...
overlap_lasso <- function(X, Y, n = dim(X)[1], p = dim(X)[2], lambda_min = 0.05, frac_overlap_allowed = 0.05, halt_error_diff=1.0001) {
    Y = as.matrix(Y)
    if (!dim(Y)[1] == n) {
        stop("Y does not have the same number of rows as X, or the format is wrong")
    }

    result = .Call(lasso_, X, Y, lambda_min, max(abs(apply(X,2, `%*%`, Y))), frac_overlap_allowed, halt_error_diff)
