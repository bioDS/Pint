    ...
    SEXP max_nz_beta_, SEXP max_lambdas_, SEXP verbose_, SEXP log_filename_)
{
    ...
    char* log_filename = CHAR(STRING_ELT(filename, 0));

    int job_args_num = 4;
    char* job_args[job_args_num];
    job_args[0] = sprintf("%d", n);
    job_args[1] = sprintf("%d", p);
    job_args[2] = sprintf("%d", max_interaction_distance);
    job_args[3] = sprintf("%d", max_lambdas);
