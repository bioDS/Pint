...
__kernel void vector_add(__global const long *A, __global const long *B,
                         __global long *C) {
...
__kernel void
update_working_set(__global float *rowsum, __global long *wont_update, long p,
                   long n, float lambda, __global robin_hood::unordered_flat_map<long, float> beta,
                   __global char *append, __global long *target_col_nz,
                   __global long *target_X, __global long *target_col_offsets,
                   __global long *updateable_items, long count_may_update, 
  ...
  long local_id = get_local_id(0);
  long group_size = get_local_size(0);
  long group_id = get_group_id(0);
  long num_groups = get_num_groups(0);
  ...
  //for (long i = global_id; i < p_int; i += global_size) {
  //  append[i] = 0;
  //  // remove[i] = 0;
  //}
  ...
  for (long main_i = group_id; main_i < count_may_update; main_i += num_groups) {
    ...
    for (long inter_i = main_i + local_id; inter_i < count_may_update; inter_i += group_size) {
      ...
      long col_nz = 0;
      long j_pos = 0;
      for (long i_pos = 0; i_pos < target_col_nz[main]; i_pos++) {
        long entry_i = target_X[target_col_offsets[main] + i_pos];
        long entry_j = target_X[target_col_offsets[inter] + j_pos];
        if (entry_i == entry_j) {
          sumn += rowsum[entry_i];
          col_nz++;
        }
