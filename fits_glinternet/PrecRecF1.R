...
rds_file = sprintf("dat_precrecf1_xyz%s.rds", use_xyz)
...
for (numrows in graph_numrows) { #400
  for (t in c("yes", "no")) {
    ...
    pl
    ggsave(pl, file = sprintf("PrecRecF1_n%d_t%s_large%d_xyz%s_%s.pdf", numrows, t, large_int, use_xyz, append_str), width = 5, height = 7)
  }
}
...
for (numrows in graph_numrows) { #400
  ...
  pdf(sprintf("test_analysis_n%d_large%d_xyz%s_%s.pdf", numrows, large_int, use_xyz, append_str), width = 5, height = 5)
