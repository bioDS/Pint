...
extern {
    ...
    pub fn p4ndenc128v32(in_: *const u32, n: usize, out: *mut ::std::os::raw::c_uchar) -> usize;
    pub fn p4nddec128v32(in_: *const::std::os::raw::c_uchar, n: usize, out: *mut u32) -> usize;
    pub fn p4ndenc256v32(in_: *const u32, n: usize, out: *mut ::std::os::raw::c_uchar) -> usize;
    pub fn p4nddec256v32(in_: *const::std::os::raw::c_uchar, n: usize, out: *mut u32) -> usize;
}
...
pub fn compress_delta(seq: &[u32]) -> Vec<u8> {
    ...
    let size = seq.len();
    let mut buffer = vec![0_u8; 8*size+64];
    let mut compressed_size = -1;
    //println!("size {}, capacity {}", size, buffer.capacity());
    unsafe {
        compressed_size = p4ndenc256v32(seq.as_ptr(), size, buffer.as_mut_ptr()) as isize;
    }
    assert!(compressed_size < buffer.capacity() as isize);
...
pub fn decompress_delta(arr: &[u8], size: usize, seq: &mut Vec<u32>) -> usize {
    ...
    seq.clear();
    seq.resize(2*size + 64, 0); //TODO: overkill
    unsafe {
        bytes_read = p4nddec256v32(arr.as_ptr(), size, seq.as_mut_ptr()) as isize;
    }
    //assert!((bytes_read as usize) < seq.len());
    seq.resize(size, 0);
    bytes_read as usize
}
